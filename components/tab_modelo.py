...
def render_tab_modelo(modelo: dict, df_top) -> None:
    ...
    _info_ols, _info_top = st.columns([1, 1], gap="large")
    with _info_ols:
        if not df_mod.empty:
            st.markdown(
    ...
    with _info_top:
        if not df_top.empty:
            st.markdown(
                ...
                'font-size: 0.78rem; color: rgb(148, 163, 184); line-height: 1.65; '
                'box-shadow: rgba(0, 0, 0, 0.5) 0px 4px 20px;">'
                '<b style="color: rgb(241, 245, 249); font-size: 0.82rem;">⚠️ Brecha de Capital Humano</b><br><br>'
                'Aunque el sistema incluye áreas STEM, las <b style="color:#EAB308">carreras tradicionales</b> (Administración, Derecho, Contabilidad) '
                'dominan el volumen nacional. Las ingenierías clave para la transición energética '
                '(Civil, Electrónica, Software) ocupan rangos inferiores, evidenciando un <b style="color:#F1F5F9">déficit estructural</b> '
                'de talento especializado para expandir el sector eléctrico.'
                '</div>'
                '</details>'
                '</div>',
                unsafe_allow_html=True,
